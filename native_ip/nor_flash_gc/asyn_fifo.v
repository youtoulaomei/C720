...
generate
    if(RAM_STYLE=="REG")
        begin
            (* ram_style="register" *)reg        [DATA_WIDTH-1:0]       mem [DATA_DEEPTH-1:0]/* synthesis syn_ramstyle="registers" */;
    ...
    else if(RAM_STYLE=="BRAM")
        begin
            (* ram_style="block" *)reg        [DATA_WIDTH-1:0]       mem [DATA_DEEPTH-1:0]/* synthesis syn_ramstyle="block_ram" */;
            always @ (posedge wr_clk)
            begin
                if(wr_en == 1'b1)     
                    mem[wbin] <= #U_DLY din;    
