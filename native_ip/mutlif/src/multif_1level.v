...
parameter                           FIFO_W     = 64,
parameter                           FIFO_DEEPTH= 512,
parameter                           FIFO_ADDR_W= 9,
parameter    [FIFO_ADDR_W-1:0]      PROG_FULL_LEVEL=9'd500
)
(
input                               clk_w,
input                               clk_r,
...
output reg [FIFO_W-1:0]             out_data,
...
localparam    [FIFO_ADDR_W-1:0]     PROG_EMPTY_LEVEL = FRAME_LEN/FIFO_BW;
...
reg     [USER_NUM-1:0]              in_data_rd;
reg     [USER_NUM_W-1:0]            frm_num;
reg                                 frm_done;
reg     [FIFO_ADDR_W-1:0]           rd_cnt;
reg                                 frm_rd;
reg                                 frm_rd_r;
reg     [FIFO_W+INFO_W-1:0]         out_data_w1;
reg     [FIFO_ADDR_W-1:0]           out_cnt;
...
(
    ...
    .prog_full_thresh           (PROG_FULL_LEVEL            ),
...
integer i;
    if(rst == 1'b1)
        begin
            ...
            frm_rd        <= #U_DLY 1'b0;
            frm_rd_r      <= #U_DLY 1'b0;
            out_data_w1   <= #U_DLY 'd0;
            out_cnt       <= #U_DLY 'd0;
            in_data_rd    <= #U_DLY 'b0;
            len_fifo_rd   <= #U_DLY 'b0; 
        ...
        begin
            ...
            if(frm_rd==1'b1 && rd_cnt>=PROG_EMPTY_LEVEL-1)
                frm_done <= #U_DLY 1'b1;
            else
                frm_done <= #U_DLY 1'b0;
            ...
            if(frm_rd==1'b1 && rd_cnt>=PROG_EMPTY_LEVEL-1)
                ...
                rd_cnt <= #U_DLY rd_cnt + 'd1;

            if((frm_rd==1'b1 && rd_cnt>=PROG_EMPTY_LEVEL-1) || (frm_done==1'b1))
                frm_rd <= #U_DLY 1'b0;
            else if(frm_state==SEND_FRM && out_pfull==1'b0)
                frm_rd <= #U_DLY 1'b1;
            else
                frm_rd <= #U_DLY 1'b0;

            frm_rd_r <= #U_DLY frm_rd;
            out_data_vld <= #U_DLY frm_rd_r;

            if(frm_rd==1'b1)
                out_data_w1 <= #U_DLY in_data_dout[(FIFO_W+INFO_W)*frm_num+:(FIFO_W+INFO_W)];
            
            out_data <= #U_DLY out_data_w1; 


            if(frm_rd_r==1'b1 && out_cnt>=PROG_EMPTY_LEVEL-1)
                out_cnt <= #U_DLY 'd0;
            else if(frm_rd_r==1'b1)
                out_cnt <= #U_DLY out_cnt + 'd1;
            
            if(out_data_sof==1'b1)
                out_data_sof <= #U_DLY 1'b0;
            else if(frm_rd_r==1'b1 && out_cnt=='d0)
                out_data_sof <= #U_DLY 1'b1;

            if(out_data_eof==1'b1)
                out_data_eof <= #U_DLY 1'b0;
            else if(frm_rd_r==1'b1 && out_cnt==PROG_EMPTY_LEVEL-1)
                out_data_eof <= #U_DLY 1'b1;

            if(out_data_eof==1'b1 && out_data_end==1'b1)
                out_data_end <= #U_DLY 1'b0;
            else if(frm_rd==1'b1 && rd_cnt=='d1)
                out_data_end <= #U_DLY len_fifo_rdata[16*frm_num+15];
            
            if(frm_rd==1'b1 && rd_cnt=='d1)
                out_data_len <= #U_DLY len_fifo_rdata[(16)*frm_num+:15];

            if(frm_rd==1'b1 && rd_cnt=='d1)
                out_data_info <= #U_DLY in_data_dout[((FIFO_W+INFO_W)*frm_num+FIFO_W)+:INFO_W];

            for(i=0;i<USER_NUM;i=i+1)
            begin 
                if((frm_rd==1'b1 && rd_cnt>=PROG_EMPTY_LEVEL-1) || (frm_done==1'b1))
                    in_data_rd[i]  <= #U_DLY 1'b0;
                else if(frm_state==SEND_FRM && out_pfull==1'b0 && i==frm_num)
                    in_data_rd[i]  <= #U_DLY 1'b1;
                else
                    in_data_rd[i]  <= #U_DLY 1'b0;

                if(frm_rd==1'b1 && rd_cnt=='d1 && frm_num==i)
